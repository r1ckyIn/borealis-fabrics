...
> **为什么需要人工干预？**
>
> CLAUDE.md 的规则太多，Claude 容易"选择性遵守"，比如忘记创建 feature 分支、跳过测试等。
> **解决方案**：每轮对话只给一个明确指令，在关键节点人工确认后再继续。

### 核心原则

```
❌ 不要：一次性说"把整个模块做完"
✅ 要做：每轮只给一个明确指令，确认后再进入下一轮

❌ 不要：让 Claude 自己决定什么时候切分支、提交
✅ 要做：分支操作自己做，提交时明确指令
```
...
#### 轮次 0：开始前（你自己做，不要让 Claude 做）

```bash
# 1. 确保在正确分支
git checkout -b feature/xxx-module  # 新模块
# 或
git checkout feature/xxx-module     # 继续开发
...
**关键**：分支操作永远自己做，不要让 Claude 执行 git checkout/branch 命令。
...
审查通过后，执行 git add 和 git commit
提交消息格式：feat/fix/test(module): 描述
```

**检查点（你自己验证）**：
```bash
git log -1 --oneline  # 确认提交消息正确
git diff HEAD~1 --stat  # 确认提交的文件正确
```
...
**你说**：
```
更新 CLAUDE.md 的进度表，将功能 2.x.x 标记为完成
```
...
**你自己执行**：
```bash
git push -u origin feature/xxx-module
```

**然后告诉 Claude**：
```
创建 PR：feature/xxx-module → main
PR 标题和描述要清晰总结本模块的所有功能
```
...
**Review 发现问题后**：
```
修复 code-review 发现的问题，每修复一个就提交一次
提交消息：fix(review): 具体修复内容
```
...
git checkout main
git pull
git branch -d feature/xxx-module
```
...
│                    单功能开发 6 轮对话                          │
├──────────┬─────────────────────────────────────────────────────┤
│ 轮次 0   │ [你自己] git checkout -b feature/xxx                │
├──────────┼─────────────────────────────────────────────────────┤
│ 轮次 1   │ "规划功能 2.x.x，不要写代码"                        │
│          │ → 检查计划 → "计划批准，进入下一步"                 │
├──────────┼─────────────────────────────────────────────────────┤
│ 轮次 2   │ "编写测试，确认测试失败"                            │
│          │ → 检查测试 → "测试编写完成，开始实现"               │
...
│          │ → 检查实现 → "实现完成，运行验证循环"               │
├──────────┼─────────────────────────────────────────────────────┤
│ 轮次 4   │ "运行 build/test/lint 验证循环"                     │
│          │ → 全部通过 → "验证通过，进行代码审查"               │
├──────────┼─────────────────────────────────────────────────────┤
│ 轮次 5   │ "代码审查 + git commit"                             │
│          │ → 检查提交 → "提交完成，更新文档"                   │
├──────────┼─────────────────────────────────────────────────────┤
│ 轮次 6   │ "更新 CLAUDE.md 进度表"                             │
│          │ → 功能完成 → 回到轮次 1 做下一个功能                │
├──────────┼─────────────────────────────────────────────────────┤
│ 模块完成 │ [你自己] git push                                   │
│          │ "创建 PR" → "/code-review" → 修复 → 合并            │
└──────────┴─────────────────────────────────────────────────────┘
```
...
|------|------|
| 每轮一指令 | 不要一次性说"做完整个模块" |
| 分支自己管 | git checkout/branch/push 自己执行 |
| 确认再继续 | 每轮结束说"下一步"才进入下一轮 |
| 失败就停 | 验证失败不要跳过，修复后重跑 |
...
| Claude 的错误行为 | 你应该说 |
|------------------|---------|
| 直接在 main 分支开发 | "停，先切到 feature 分支" |
| 跳过测试直接写代码 | "先写测试，确认失败后再实现" |
| 一次性实现多个功能 | "一次只做一个功能" |
| 验证失败后继续提交 | "先修复验证问题" |
| 忘记更新文档 | "更新 CLAUDE.md 进度表" |
...
- **已完成功能**:
  - SupplierModule 全部功能 (2.1.1 - 2.1.8) ✅
  - CustomerModule CRUD 功能 (2.2.1 - 2.2.7) ✅
- **当前状态**: CustomerModule 待 E2E 集成测试
- **下一个功能**: 2.2.8 CustomerModule 集成测试
