...
每个 Session = Claude 帮你实现一个功能模块。流程如下：
...
1. 告诉 Claude 要做什么              ─→
   "开始阶段 2 的 SupplierModule"
                                    ...
                                    ←─ 2. Claude 制定计划，征求你的同意

3. 审查计划，说"可以"或提出修改      ─→

                                    ←─ 4. Claude 写测试 → 写代码 → 验证
                                       （自动运行 build/test/lint）
...
- ❌ 不需要自己写代码
- ❌ 不需要手动运行 lint/test/build（Claude 会做）
...
**Claude 做什么**：按顺序实现 Supplier → Customer → Fabric → File 四个模块的 CRUD API，包含完整的测试。
...
**预计需要 4 个 Session**（每个模块一个 Session）
