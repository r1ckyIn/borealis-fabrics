...
铂润面料是一家面料贸易中间商，拥有约 100 家供应商和 50 家 B 端客户，月均处理 500+ 订单。本项目为其建设数字化管理系统，一期为内部供应链自动化系统（H5 工作台 + 企业微信），二期为面向客户的 B2B 电商平台（微信小程序）。
...
### 3.3 前端技术选型（一期 H5 工作台）

| 技术 | 选型 | 选型理由 |
|------|------|----------|
| 框架 | React 18+ | 生态成熟，组件丰富 |
| 语言 | TypeScript (strict) | 类型安全，与后端类型共享 |
| UI 组件库 | Ant Design Mobile 5 | 移动端优先的 UI 组件库 |
...
| Vite | 5+ | 构建工具 |
| Ant Design Mobile | 5+ | 移动端 UI 组件库 |
...
1. 在企业微信管理后台创建 H5 应用
2. 配置应用主页 URL 为 H5 工作台地址
3. 配置 OAuth 授权回调域名
4. 前端引入企业微信 JSSDK

#### 8.3.2 JSSDK 使用场景（MVP）

MVP 阶段仅使用 JSSDK 进行 OAuth 授权，不使用其他 JSSDK 能力（如分享、扫码等）。

#### 8.3.3 响应式设计

- **移动优先**：主要在企业微信内嵌浏览器使用
- **兼容电脑**：PC 端企业微信也可访问，适配宽屏布局
- 断点策略：
  - 移动端：< 768px（主要场景）
  - 平板/PC：>= 768px
...
| 前端移动端兼容性 | 中 | 中 | 使用 Ant Design Mobile 成熟组件库，多设备测试 |
